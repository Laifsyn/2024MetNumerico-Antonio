...
public class DrawTable {
    ...
    private boolean separar_cada_fila = false;
    ...
    public void insertar_fila(String[] row) {
        ...
        if (this.cantidad_columnas != row.length) {;
            throw new IllegalArgumentException("La fila no tiene la misma cantidad de columnas que las anteriores : " + this.table.size());
        }
        this.table.add(row);
    }

    public void separar_cada_fila(){
        this.separar_cada_fila = true;
    }
    public String toString() {
        ...
        for (int indice_fila = 0; indice_fila < this.table.size(); indice_fila++) {
            ...
            sb.append("\n");
            if ((indice_fila == 0 || this.separar_cada_fila && indice_fila < this.table.size() - 1)) {
